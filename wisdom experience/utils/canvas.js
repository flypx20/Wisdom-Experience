export default{ 
  ...
  options:{
    ...
    run(c, w, h, num) {
      ...
      that.data.ctx2.arc(w, h, w - 50, 0, num); //每个间隔绘制的弧度
      that.data.ctx2.setStrokeStyle("black");
  ...
  canvasTap(start, end, time, w, h) {
    var that = this;
    var num = 0;
    num = num + (2 * Math.PI / 30);
    start--;
    if (start < end) {
      return false;
    }
    that.run(start, w, h,num);
    setTimeout(function () {
   ...
   */
    draw: function (id, percent, animTime) {
      ...
      var time = that.data.animTime / 30;
      wx.createSelectorQuery().select('#'+id).boundingClientRect(function (rect) { //监听canvas的宽高
        ...
        that.canvasTap(30, that.data.percentage, time, w, h)
      }).exec();
    },
  } 
}